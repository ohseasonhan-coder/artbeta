...
무료 베타에서는 별도 과금 없이 제공되는 네이버·YouTube API 할당량만 선택적으로 사용합니다. Google Custom Search는 신규 무료 연동 대상에서 제외합니다. 로컬에서는 `.env.local`, Vercel에서는 Project Settings → Environment Variables에 사용할 검색원의 키를 추가하고 재배포합니다. 모든 검색원을 한꺼번에 설정할 필요는 없습니다.
