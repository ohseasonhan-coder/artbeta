...
디자인 단계의 `외부 공개 기록으로 프로필 보강`은 기존 Google Custom Search 설정을 이용해 `namu.wiki`, `otr.co.kr`, `showgle.co.kr`의 공개 검색 결과를 찾습니다. Gemini는 검색 제목과 요약문에 명시된 내용만 경력 후보로 구조화합니다. 사용자가 `원문 확인` 후 `출처를 확인했으며 반영`을 눌러야 연혁과 PPT 근거에 추가됩니다. 나무위키는 참고 자료, OTR·쇼글은 플랫폼 등록 자료로 구분되며 원문 링크가 PPT 발표자 메모에 남습니다.
