...
- 전체 후보 중 최대 4장의 핵심 이미지만 골라 표지·소개·대표 장면에 한 장씩 배치
...
디자인 단계의 `무료 외부 기록 검색`은 유료 검색 API를 호출하지 않습니다. 앱이 활동명과 식별 정보를 포함한 나무위키·OTR·쇼글 검색 바로가기를 만들고, 사용자가 원문에서 동일 인물임을 확인한 뒤 링크와 경력·수상·공연 내용을 한 줄씩 입력합니다. 입력한 내용은 즉시 연혁과 PPT 근거에 추가됩니다. 나무위키는 참고 자료, OTR·쇼글은 플랫폼 등록 자료로 구분되며 원문 링크가 PPT 발표자 메모에 남습니다. 검색 API 비용은 발생하지 않습니다.
