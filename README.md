...
PPTX 다운로드 시에는 Gemini를 다시 호출합니다. Gemini가 업로드된 이미지 썸네일과 확정된 프로필 사실을 보고 섭외 담당자를 위한 이야기 흐름, 이미지별 역할, 슬라이드 유형과 레이아웃을 정합니다. 브라우저에서는 이 기획과 원본 이미지를 사용해 PowerPoint에서 수정 가능한 PPTX를 생성합니다. AI를 사용할 수 없을 때만 동일한 자산을 활용하는 기본 기획으로 대체합니다.
