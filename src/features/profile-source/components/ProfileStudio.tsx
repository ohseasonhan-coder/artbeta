  ...
  const prepareDeck = async () => {
    ...
    try {
      ...
      setNotice(prepared.meta.mode === "ai"
        ? `${prepared.meta.provider} · ${prepared.meta.model}로 자료량에 맞춰 ${prepared.plan.slides.length}페이지를 구성했어요.${addedWebImageCount ? ` 검수된 웹 사진 ${addedWebImageCount}장 자동 추가 ·` : ""}${generatedImageCount ? ` AI 연출 이미지 ${generatedImageCount}장 자동 보충 ·` : ""}${prepared.meta.visualReviewIterations ? " Gemini 시각 검수 완료 ·" : ""} 목적별 경력 ${prepared.meta.coveredFactCount ?? 0}/${prepared.meta.totalFactCount ?? 0}개 반영 · 사진 반복과 텍스트 이탈 방지 적용.${prepared.meta.releaseReady ? " 다운로드할 수 있습니다." : " 일부 항목은 다시 검수가 필요합니다."}`
        : prepared.meta.errorCode === "GEMINI_QUOTA_EXCEEDED"
          ? "Gemini 무료 사용량이 잠시 소진되어 기본 미리보기로 전환했어요. 사용량이 초기화된 뒤 다시 자동 검수를 실행해 주세요."
          : "Gemini 기획을 완료하지 못해 기본 미리보기로 전환했어요. 잠시 후 다시 시도해 주세요.");
      setStep(2);
    } catch {
