  ...
  try {
    ...
    if (!process.env.GEMINI_API_KEY) {
      return NextResponse.json({
        mode: "heuristic",
        classifications: images.map((image) => ({ id: image.id, role: fallbackRole(image), relevanceScore: 0.65, qualityScore: 0.65, reason: "이미지 비율과 페이지 문맥으로 자동 분류" })),
      });
    }

    const parts: Part[] = [{
      text: `문화예술인 '${body.artistName || "이름 미상"}'의 기존 PDF·PPTX에서 분리한 이미지를 새 제안서용 자산으로 분류하세요. 분야는 '${body.primaryField || "미지정"}'입니다. 각 이미지를 정확히 하나의 역할로 분류합니다. portrait=얼굴·인물·단체가 주제인 대표사진, stage=공연·전시·연주·창작·관객 반응 등 실제 활동 장면, poster=행사 포스터·홍보물·타이포그래피 중심 그래픽, history=연혁·수상·보도·인증 자료, other=작품·장소 등 보조 자산, exclude=로고·아이콘·QR·서명·장식 배경·문서 전체 캡처·UI·중복·저화질·주제 불명 이미지입니다. 페이지 문맥과 이미지 자체를 함께 보세요. 포스터와 연혁 자료는 잘리지 않게 사용해야 하므로 사진으로 잘못 분류하지 마세요. relevanceScore는 해당 아티스트 제안서와의 직접 관련성, qualityScore는 해상도·구도·가독성을 평가합니다. 추측으로 인물 신원을 확정하지 말고 보이는 역할만 분류하세요.`,
    }];
    ...
    return NextResponse.json({
      mode: "ai",
      classifications: images.map((image) => classified.get(image.id) || ({ id: image.id, role: fallbackRole(image), relevanceScore: 0.6, qualityScore: 0.6, reason: "AI 응답 누락으로 문맥 기반 분류" })),
    });
  } catch (error) {
