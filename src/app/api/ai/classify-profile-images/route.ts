...
export async function POST(request: Request) {
  try {
    ...
    const parts: Part[] = [{
      text: `문화예술인 '${body.artistName || "이름 미상"}'의 기존 PDF·PPTX에서 분리한 이미지를 새 제안서용 자산으로 분류하세요. 분야는 '${body.primaryField || "미지정"}'입니다. 각 이미지를 정확히 하나의 역할로 분류합니다. portrait=얼굴·인물·단체가 주제인 대표사진, stage=공연·전시·연주·창작·관객 반응 등 실제 활동 장면, poster=행사 포스터·홍보물·타이포그래피 중심 그래픽, history=연혁·수상·보도·인증 자료, other=작품·장소 등 보조 자산, exclude=로고·아이콘·QR·서명·장식 배경·문서 전체 캡처·UI·중복·저화질·주제 불명 이미지입니다. 여러 사진을 이어 붙인 콜라주, 영상 썸네일 합성, 서로 다른 시점·의상·인물이 한 화면에 분할된 몽타주, 원본 위에 큰 제목이 얹힌 홍보 이미지는 최종 제안서 사진으로 사용하지 말고 role=exclude로 지정하세요. 단체사진은 페이지 문맥에서 해당 아티스트가 명시되고 얼굴과 활동 장면이 충분히 선명할 때만 portrait 또는 stage로 허용합니다. 전체 후보를 서로 비교해 같은 사진, 같은 장면의 재크롭·축소본, 사실상 동일한 포스터가 있으면 가장 선명한 한 장만 유지하고 나머지는 role=exclude로 지정하며 duplicateOf에 유지할 이미지 ID를 기록하세요. 중복이 아니면 duplicateOf=null입니다. 페이지 문맥과 이미지 자체를 함께 보세요. 포스터와 연혁 자료는 잘리지 않게 사용해야 하므로 사진으로 잘못 분류하지 마세요. relevanceScore는 해당 아티스트 제안서와의 직접 관련성, qualityScore는 해상도·구도·가독성을 평가합니다. 추측으로 인물 신원을 확정하지 말고 보이는 역할만 분류하세요.`,
    }];
